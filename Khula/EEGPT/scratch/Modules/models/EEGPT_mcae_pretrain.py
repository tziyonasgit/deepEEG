...
# CHANNEL_DICT will be the same as channel ordering in the input data
CHANNEL_DICT = {k.upper(): v for v, k in enumerate(['FPZ', 'FP2', 'AF3', 'AF4', 'F7', 'F3', 'F1', 'FZ', 'F2', 'F4', 'F8', 'FT7', 'FC5', 'FC3', 'FC1', 'FCZ', 'FC2', 'FC4', 'FC6', 'FT8', 'T7', 'C5', 'C3', 'C1', 'C2', 'C4',
...
class EEGPTClassifier(nn.Module):
    def __init__(self, models_configs, max_epochs, timepoints, steps_per_epoch, USE_LOSS_A=True, USE_LN=True, USE_SKIP=True):
        super().__init__()
    ...
    def forward(self, x, chan_ids=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        print("ENC OUT (before flatten):", x.shape)
        x = x.flatten(1)
        print("##################DEBUG:", x.shape)
        x = self.head(x)
        return x
