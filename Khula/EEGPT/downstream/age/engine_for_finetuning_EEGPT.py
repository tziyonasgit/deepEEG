...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        features = features.view(64, numPatches, 4, 512)
...
def evaluate(data_loader, model, device, args, header='Test:', ch_names=None, metrics=['acc'], is_binary=True):
    ...
    for step, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        if is_binary:
            ...
            target = target.float().unsqueeze(1)  # expects [B,1]
        else:
            target = target.to(device, non_blocking=True)
            target = target.long().squeeze(-1)  # expects [B]
        ...
        results = utils.get_metrics(
            probs.detach().cpu().numpy(), target.detach().cpu().numpy(), metrics, is_binary)

        # collecting prediction probabilities and true labels
        pred.append(probs.detach().cpu())
        true.append(target.detach().cpu())

        # converting to lists for CSV output
