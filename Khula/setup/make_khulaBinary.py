...
def splitSrcDest(line):
    ...
    # src:  /scratch/chntzi001/khula/3M/1_191_96928021_3_20230125_100944002_processed.set
    # dest:  chntzi001/khula/processed/train
    src = parts[0]
    dst = "/scratch/" + parts[1]
    return src, dst
...
if __name__ == "__main__":
    ...
    with open("trainBinary.txt") as f:
        train_files = [line.strip() for line in f]

    with open("valBinary.txt") as f:
    ...
    for train_sub in train_files:
        ...
        if match.group(1) in ["3", "6"]:
            label = 0
        else:
            label = 1
        parameters.append(
            [train_sub_src, train_dump_folder, label, chOrder_standard, egiStandardDict])
    ...
    for val_sub in val_files:
        ...
        if match:
            if match.group(1) in ["3", "6"]:
                ...
                label = 1
            parameters.append(
                [val_sub_src, val_dump_folder, label, chOrder_standard, egiStandardDict])
        else:
    ...
    for test_sub in test_files:
        ...
        if match.group(1) in ["3", "6"]:
            ...
            label = 1
        parameters.append(
            [test_sub_src, test_dump_folder, label, chOrder_standard, egiStandardDict])
