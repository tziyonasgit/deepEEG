...
def get_metrics(output, target, metrics, is_binary, threshold=0.5):
    ...
    if is_binary:
        ...
        print("y_true: ", target)
        print("y_true shape: ", target.shape)
        print("y_prob: ", output)
        print("y_prob shape: ", output.shape)
...
@torch.no_grad()
def evaluate(data_loader, model, device, step_fn, header='Test:', metrics=['acc'], is_binary=False, use_amp=True):
