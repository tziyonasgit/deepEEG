...
def callback(res):
    print('<进程%s> subject %s accu %s' %
          (os.getpid(), res['sub'], str(res["accu"])))
...
def geban_entropy(batch_size=10, n_class=4, entropy_scope=[0, 1]):
    while True:
        ...
        if entropy_scope[0] <= ent and ent <= entropy_scope[1]:
            break
    return num_class
...
def get_subj_data(sub, data_path, few_shot_number=1, is_few_EA=False, target_sample=-1, sess=None, use_average=False):
    ...
    if sess is not None:
        ...
        train_path = os.path.join(
            data_path, r'sub{}_sess{}_train/Data.mat'.format(i, sess))
        test_path = os.path.join(
            data_path, r'sub{}_sess{}_test/Data.mat'.format(i, sess))
    else:
    ...
    if use_average:
        train_data['x_data'] = train_data['x_data'] - \
            train_data['x_data'].mean(-2, keepdims=True)
    ...
    if use_average:
        test_data['x_data'] = test_data['x_data'] - \
            test_data['x_data'].mean(-2, keepdims=True)
    ...
    train_x, valid_x, train_y, valid_y = train_test_split(
        session_2_x, session_2_y, test_size=0.1, stratify=session_2_y)
...
def get_data(sub, data_path, few_shot_number=1, is_few_EA=False, target_sample=-1, use_avg=True, use_channels=None):
    ...
    if target_sample > 0:
        test_x_1 = temporal_interpolation(
            test_x_1, target_sample, use_avg=use_avg)
        test_x_2 = temporal_interpolation(
            test_x_2, target_sample, use_avg=use_avg)
    if use_channels is not None:
        test_dataset = eeg_dataset(torch.cat([test_x_1, test_x_2], dim=0)[
                                   :, use_channels, :], torch.cat([test_y_1, test_y_2], dim=0))
    else:
        test_dataset = eeg_dataset(
            torch.cat([test_x_1, test_x_2], dim=0), torch.cat([test_y_1, test_y_2], dim=0))
    ...
    if target_sample > 0:
        source_train_x = temporal_interpolation(
            source_train_x, target_sample, use_avg=use_avg)
        source_valid_x = temporal_interpolation(
            source_valid_x, target_sample, use_avg=use_avg)

    if use_channels is not None:
        train_dataset = eeg_dataset(
            source_train_x[:, use_channels, :], source_train_y, source_train_s)
    else:
        train_dataset = eeg_dataset(
            source_train_x, source_train_y, source_train_s)

    if use_channels is not None:
        valid_datset = eeg_dataset(
            source_valid_x[:, use_channels, :], source_valid_y, source_valid_s)
    else:
        valid_datset = eeg_dataset(
...
def get_data_openbmi(data_path, few_shot_number=1, is_few_EA=False, target_sample=-1):
    ...
    for sub in test_subs:
        target_session_1_path = os.path.join(
            data_path, r'sub{}_sess1_train/Data.mat'.format(sub))
        target_session_2_path = os.path.join(
            data_path, r'sub{}_sess2_train/Data.mat'.format(sub))
    ...
    test_dataset = eeg_dataset(
        torch.cat(source_test_x, dim=0), torch.cat(source_test_y, dim=0))

    source_train_x = []
    source_train_y = []
    for i in train_subs:
        train_path = os.path.join(
            data_path, r'sub{}_sess1_train/Data.mat'.format(i))
        train_data = sio.loadmat(train_path)

        test_path = os.path.join(
            data_path, r'sub{}_sess2_train/Data.mat'.format(i))
        test_data = sio.loadmat(test_path)
...
def get_data_Nakanishi2015(sub, data_path="Data/Nakanishi2015_8_64HZ/", few_shot_number=1, is_few_EA=False, target_sample=-1):
    ...
    for i in tqdm(range(1, 11)):
        ...
        train_x, valid_x, train_y, valid_y = train_test_split(
            session_1_x, session_1_y, test_size=40, stratify=session_1_y)
...
if __name__ == "__main__":
    train_dataset, valid_dataset, test_dataset = get_data_Wang2016(
        1, "Data/Wang2016_4_20HZ/", 1, is_few_EA=False, target_sample=-1)
...
def min_max_normalize(x: torch.Tensor, data_max=None, data_min=None, low=-1, high=1):
    if data_max is not None:
        ...
        scale = 2 * \
            (torch.clamp_max((x.max() - x.min()) / max_scale, 1.0) - 0.5)
    ...
    if len(x.shape) == 2:
        ...
        xmin = torch.min(torch.min(x, keepdim=True, dim=1)
                         [0], keepdim=True, dim=-1)[0]
        xmax = torch.max(torch.max(x, keepdim=True, dim=1)
                         [0], keepdim=True, dim=-1)[0]
...
# -- read Kaggle ERN
ch_names_kaggle_ern = list(
    "Fp1,Fp2,AF7,AF3,AF4,AF8,F7,F5,F3,F1,Fz,F2,F4,F6,F8,FT7,FC5,FC3,FC1,FCz,FC2,FC4,FC6,FT8,T7,C5,C3,C1,Cz,C2,C4,C6,T8,TP7,CP5,CP3,CP1,CPz,CP2,CP4,CP6,TP8,P7,P5,P3,P1,Pz,P2,P4,P6,P8,PO7,POz,PO8,O1,O2".split(','))
...
def read_csv_epochs(filename, tmin, tlen, use_channels_names=use_channels_names, data_max=None, data_min=None):
    ...
    if use_channels_names is not None:
        ...
        for ch in use_channels_names:
            choice_channels.append([x.lower().strip('.')
                                   for x in ch_names_kaggle_ern].index(ch.lower()))
        use_channels = choice_channels
    if data_max is not None:
        use_channels += [-1]

    def xform(x): return min_max_normalize(x, data_max, data_min)
...
def read_kaggle_ern_test(
    ...
    labels = pd.read_csv(os.path.join(
        path, 'KaggleERN', 'true_labels.csv'))['label']
    ...
    for i in tqdm.tqdm(subjects):
        for j in sessions:
            filename = os.path.join(
                path, "KaggleERN", "test", "Data_S{:02d}_Sess{:02d}.csv".format(i, j))
...
def read_kaggle_ern_train(
    ...
    for i in tqdm.tqdm(subjects):
        for j in sessions:
            if i > 9:
                if (i == 22 and j == 5):
                    print("Skipped error file " + "KagglERN/train/Data_S" +
                          str(i)+"_Sess0"+str(j)+".csv")
                else:
                    filename = os.path.join(
