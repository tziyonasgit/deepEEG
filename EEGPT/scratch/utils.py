...
def prepare_KHULA_dataset(root, numClasses):
    ...
    print(len(train_files), len(val_files), len(test_files))
    return train_dataset, test_dataset, val_dataset, steps_per_epoch
...
def prepare_TUSZ_dataset(root):
    ...
    train_csv = pd.read_csv(
        '/disks/SSD2/data/transformed2/tuh-eeg/seizures.csv')
    all_eegs = np.load(
        '/disks/SSD2/data/transformed2/tuh-eeg/eegs.npy', allow_pickle=True).item()
    ...
    train_df, valid_df, _, _ = train_test_split(
        train_df, train_df['class_code'], test_size=0.2, random_state=42, shuffle=True, stratify=train_df['class_code'])
