...
class PatchNormEmbed(nn.Module):
    ...
    def __init__(self, img_size=(64, 1000), patch_size=16, patch_stride=None, embed_dim=768):
        ...
        self.unfold = torch.nn.Unfold(kernel_size=(1, patch_size), stride=(
            1, patch_stride if patch_stride is not None else patch_size))
...
class EEGTransformerReconstructor(nn.Module):
    ...
    def forward(self, x, chan_ids=None, mask_x=None, mask_y=None):
        ...
        chan_embed = self.chan_embed(
            chan_ids).unsqueeze(0)  # (1,C) -> (1,1,C,D)
        # -- get freqs for RoPE

        if mask_x is not None:
            mask_x = mask_x.to(x.device)
            # select first as represent
        ...
        if mask_y is not None:
            ...
            freqs = self.time_embed.prepare_freqs(
                (C, N), x.device, x.dtype)  # NC, time_dim
            ...
            if self.use_pos_embed:
                x = x + \
                    self.pos_embed.repeat((B, x.shape[1], 1, 1)).to(x.device)
...
class EEGTransformerPredictor(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if use_pos_embed:
            self.pos_embed = nn.Parameter(torch.zeros(
                1, 1, embed_num, predictor_embed_dim))
            trunc_normal_(self.pos_embed, std=init_std)
    ...
    def forward(self, x, mask_x=None, mask_t=None):
        ...
        if mask_x is not None:
            ...
            if mask_t is None:
                mask_t = torch.tensor(
                    list(set(list(range(0, N))) - set(mask_x.tolist()))).long()
            # -- concat query mask_token ys
            N_y = mask_t.shape[0]
...
class EEGTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # --
        # is this where electrode graph network is applied???
        # We initially construct a graph network in which each node represents an
        # electrode utilized during the pre-training stage. The total number of nodes is denoted by |R(X)|.
        # Benefiting from the comprehensive data coverage in the pre-training, these nodes include nearly all
        # electrodes commonly employed, encompassing those found in Y . Each node is represented by a
        # learnable vector of length C. They form a fully interconnected graph G ∈ R
        # |R(X)|×C . For each zj ,
        # the electrodes it comprises are mapped to a subgraph Gj within G. Upon the introduction of z_j into
        # the network, only the nodes contained within the subgraph \protect \mathcal {G}_j are activated. This activation process
        # involves updating the representations of these specific nodes by adding the corresponding elem
