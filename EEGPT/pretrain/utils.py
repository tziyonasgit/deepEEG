"""
Utilities for EEGPT pretraining
================================================================
...
def InfoNCELoss(pred, target, t=0.):
    """
    Compares two sets of feature vectors and computes the loss between them
    Args:
        pred (torch.Tensor): prediction feature vector
        target (torch.Tensor): target feature vector
        t (float, optional): temperature scaling

    Returns:
        Tuple[torch.Tensor, torch.Tensor]: mean loss and mean accuracy
    """
    # B, NC, D
    NC, B, D = pred.shape
    similarity = torch.matmul(
        pred, target.transpose(1, 2)) * math.exp(t)  # NC, B, B
...
def BatchMAE_InfoNCELoss(pred, target, t=0.):
    """
    Runs InfoNCE two ways on (B, N, D) tensors: across time and across batches
...
def CoupleInfoNCELoss(pred, target, t=0.):
    """
    Runs three InfoNCE comparisons on (B, N, C, D) tensors: Batch view, time view and channel view
...
def _generate_negatives(z, num_negatives=20):
    ...
    with torch.no_grad():
        ...
        negative_inds = torch.randint(0, full_len, size=(
            batch_size, full_len * num_negatives))
    ...
    z_k = z_k[negative_inds.view(-1)].view(batch_size,
                                           full_len, num_negatives, feat)
...
def _calculate_similarity(z, c, negatives, temp=0.1):
    ...
    # negatives不含冲突的损失项
    negative_in_target = (c == negatives).all(
        dim=-1) | (z == negatives).all(dim=-1)
...
class SelfSuperviseLoss():
    ...
    def __call__(self, dec_data, enc_data):
        negatives, _ = _generate_negatives(
            enc_data, num_negatives=self.num_negatives)
...
class WarmupCosineSchedule(object):
    ...
    def step(self):
        ...
        if self._step < self.warmup_steps:
            ...
            progress = float(self._step - self.warmup_steps) / \
                float(max(1, self.T_max))
...
class CosineWDSchedule(object):
    """
    Smoothly moves from ref_wd to final_wd with a cosine curve
    """
