...
from engine_for_finetuning_reg import train_one_epoch, evaluate, getVarOutputs
from utils import NativeScalerWithGradNormCount as NativeScaler
import utils
from Modules.models.EEGPT_bayley import EEGPTRegression, getOutput
...
def get_args():
    ...
    parser.add_argument(
        '--output_dir', default="/scratch/chntzi001/khula/checkpoints/finetune_khula_eegpt/bayley", type=str)
...
def get_models(args):
    ...
    print("channels names:", use_channels_names)
...
def get_dataset(args):
    if args.dataset == 'KHULA':
        ...
        train_dataset, test_dataset, val_dataset = utils.prepare_KHULA_dataset(args.task,
                                                                               filepath)
...
def main(args, ds_init):
    ...
    tsneplots = f"/scratch/chntzi001/khula/checkpoints/finetune_khula_eegpt/regression/{args.task}/{args.date}/{run_name}/tsne_plots"
    ...
    dataset_train, dataset_test, dataset_val, ch_names = get_dataset(
        args)
    ...
    with open(output_csv, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Epoch", "Prediction", "True Label"])
    ...
    if args.finetune:
        ...
        bad = [k for k in checkpoint_model.keys()
               if k.startswith("head.") or "classifier" in k or k.startswith("fc.")]
        for k in bad:
    ...
    # checking head
    print("Head requires_grad:", any(p.requires_grad for p in head.parameters()))
    head_param_ids = {id(p) for p in head.parameters()}
    in_optim = any(
        id(p) in head_param_ids for g in optimizer.param_groups for p in g["params"])
    print("Head in optimizer:", in_optim)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            val_stats = evaluate(epoch, data_loader_val, model, device, args, header='Val:',
                                 ch_names=ch_names)

            test_stats = evaluate(epoch, data_loader_test, model, device, args, header='Test:',
                                  ch_names=ch_names)
            ...
            print(f"Val   | MAE {val_stats['MAE']:.4f} | RMSE {val_stats['RMSE']:.4f} | R2 {val_stats['R2']:.4f} | "
                  f"MAE_batch_avg {val_stats['mae_batch_avg']:.4f}")
            print(f"Test  | MAE {test_stats['MAE']:.4f} | RMSE {test_stats['RMSE']:.4f} | R2 {test_stats['R2']:.4f} | "
                  f"MAE_batch_avg {test_stats['mae_batch_avg']:.4f}")
            ...
            if val_stats['MAE'] < best_mae:
                ...
                best_mae_test = test_stats["MAE"]

            if log_writer is not None:
                for key, value in train_stats.items():
                    log_writer.update(**{key: value}, head="train", step=epoch)

                # Val metrics
                log_writer.update(
                    MAE=val_stats['MAE'],   head="val",  step=epoch)
                log_writer.update(
                    RMSE=val_stats['RMSE'], head="val",  step=epoch)
                log_writer.update(
                    R2=val_stats['R2'],     head="val",  step=epoch)
                log_writer.update(
                    mae_batch_avg=val_stats['mae_batch_avg'], head="val", step=epoch)

                # Test metrics
                log_writer.update(
                    MAE=test_stats['MAE'],   head="test", step=epoch)
                log_writer.update(
                    RMSE=test_stats['RMSE'], head="test", step=epoch)
                log_writer.update(
                    R2=test_stats['R2'],     head="test", step=epoch)
                log_writer.update(
                    mae_batch_avg=test_stats['mae_batch_avg'], head="test", step=epoch)
