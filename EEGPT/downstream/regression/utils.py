...
class KHULALoader(torch.utils.data.Dataset):
    ...
    def __init__(self, task, root, files, sampling_rate=256):
        self.root = root
        self.files = files
    ...
    def __getitem__(self, index):
        if self.task == "AGE":
            age = os.path.basename(self.files[index]).split('_')[3]      # '24'
            age_int = int(age)
...
def prepare_KHULA_dataset(task, root):
    # set random seed
    seed = 12345
    np.random.seed(seed)
    ...
    train_dataset = KHULALoader(task, os.path.join(root, "train"), train_files)
    test_dataset = KHULALoader(task, os.path.join(root, "test"), test_files)
    val_dataset = KHULALoader(task, os.path.join(root, "val"), val_files)
...
def get_metrics(output, target, metrics, threshold=0.5):
