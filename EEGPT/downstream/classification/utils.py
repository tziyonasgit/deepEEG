...
class KHULAbinLoader(torch.utils.data.Dataset):
    def __init__(self, root, files, task, sampling_rate=200):
        self.root = root
        self.files = files
        self.sampling_rate = sampling_rate
        self.task = task
        ...
        keep = []
        zeros = ones = dropped = 0
        for f in files:
            try:
                s = pickle.load(open(os.path.join(root, f), "rb"))
                y = s.get("y", {}).get(task, None)
        ...
        self.files = keep
        print(
            f"[KHULAbinLoader:{task}] kept={len(self.files)} (0s={zeros}, 1s={ones}), dropped={dropped}")
...
def prepare_KHULA_dataset(root, numClasses, task):
    # set random seed
    seed = 12345
    np.random.seed(seed)
    ...
    if numClasses > 1:
        ...
        train_dataset = KHULAbinLoader(
            os.path.join(root, "train"), train_files, task=task)
        print("train dataset prepared")
        test_dataset = KHULAbinLoader(os.path.join(
            root, "test"), test_files, task=task)
        val_dataset = KHULAbinLoader(
            os.path.join(root, "val"), val_files, task=task)
