""" Generate PSD plots for 3M vs 24M data samples (AND includes adult sample data for comparison)"""
...
def mean_psd_in_folder(folder):
    ...
    if len(per_file_means) == 0:
        raise RuntimeError(
            f"No usable .set files in {folder}. Skipped {skipped} files")
